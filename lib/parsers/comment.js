 ...
 * @param  callback      {commentParserCallback} The callback function.
 */
function CommentParser (sourceComment, info, callback) {
  if (!(this instanceof CommentParser)) return new CommentParser(sourceComment, info, callback);

  var cb = !callback ? info : callback
    , comment = this.comment = callback ? clone(info) : {};
