 ...
 *   Instead of the global todos. Add comments to global namespace if no code is
 *   on the next line, like this comment
 ...
 * @param [options.name]    {String}            The name of the documentation.
 ...
 *
 * @param codeInfo {Array} The array of detected code information.
 * @param comments {Array} The array of comments.
 */
  ...
  comments.forEach(function (comment) {
    ...
    for (var i = codeInfo.length; i--;) {
      if (codeInfo[i].name === name) {
        codeInfo[i] = merge(codeInfo[i], comment);
        return;
      }
    }
...
function getExportsName (code) {
  ...
  // Return the name that is detected
...
/**
 * Get comments from the code and parse them.
 *
 * @param  code {String}              The code from the file.
 ...
 *
 * @param  code {String}              The code from the file.
 * @param  cb   {getRequiresCallback} The async callback.
 */
 ...
 *
 * @param  code {String}              The code from the file.
 * @param  cb   {getCodeInfoCallback} The async callback.
 */
function getCodeInfo (code, cb) {
  ...
  lines.forEach(function (line) {
    ...
    // Check if the line has var or function assignment
    if (line.indexOf('var') !== -1 || line.indexOf('function') !== -1) {
      var info = detectCodeInfo(line);

      // If it's contains something interesting add it to the array
      if (info.type !== 'Callback' && (info.type === 'Function' ||
          info.type === 'Constructor' || info.constant !== false ||
          info.exports !== false)) codeInfo.push(info);
    }
  });
...
function trimArray (array) {
  ...
  // Don't add empty strings and trim the string
