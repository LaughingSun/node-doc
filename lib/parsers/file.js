...
var path = require('path')
  , fs = require('fs')
  , async = require('async')
  , constructDoc = require('../construct')
  ...
  fs.readFile(file, 'utf8', function (err, code) {
    ...
    async.parallel({
      ...
      async.parallel(requireNamespaces, function (err, namespaces) {
        ...
        var doc = constructDoc(options, comments, getExportsName(code), namespaces);
...
function getRequires (code, cb) {
  ...
  while ((requirePlace = code.indexOf('require')) !== -1) { // Assignment!
    var file = getRequirePath(code.slice(requirePlace));
...
function getRequirePath (code) {
  ...
  if ((start === '\'' && end === '\'') || (start === '"' && end === '"')) {
    ...
    throw new Error('Can\'t yet deal with variables in require calls');
  }

  // Only document local files
  if (file.slice(0, 2) !== './' || file.slice(0, 3) !== '../' ||
      file.slice(0, 1) !== '/') return file;
}
