 ...
 * @public
 *
 * @param sourceTag {String}           String with a @tag in it.
 * @param cb        {parseTagCallback} The callback function.
 */
function parseTag (sourceTag, cb) {
  var tag = sourceTag.trim()
    , firstSpace = tag.indexOf(' ')
  ...
  switch (type) {
    ...
    case 'private':
      if (type !== 'access') {
        ...
        return cb(new Error('@access only supports public, private'));
      }
      break;
