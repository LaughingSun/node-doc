...
var fs = require('fs')
  ...
  , toMarkdown = require('./converters/markdown')
  , toHTML = require('./converters/html')
 ...
 * object or in Github flavored Markdown.
 *
...
function parser (options, cb) {
  ...
  fileParser(path, options, function (err, doc) {
    ...
    if (result === 'markdown')  doc = toMarkdown(doc);
    else if (result === 'html') doc = toHTML(doc);
    else                        doc = prepareJson(doc);

    if (output) return save(doc, cb);
