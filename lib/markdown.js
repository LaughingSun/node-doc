/**
 * @title To Markdown
 * @desc Parses a object into Markdown (Github flavored).
 * @public
 *
...
function toMarkdown (title, comments) {
  ...
  comments.forEach(function (comment) {
    ...
    if (comment.constant) md += '#### Constant\n\n';
    ...
    if (comment.type && comment.type.toLowerCase() === 'function') {
      ...
      if (comment.example) {
        ...
        if (comment.return) {
          example = 'var ' + comment.name.toLowerCase() + ' = ';

          if (comment.constructor === true) example += 'new ';
        }
...
/**
 * @title Create table
 * @desc Parses params or return object and it's properties into a markdown table.
 * @private
 *
